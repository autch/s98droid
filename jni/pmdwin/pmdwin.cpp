...
char* strip_ext(char* pathname, char* ext_out, size_t extout_size)
{
	char* p;

	p = strrchr(pathname, '.');
	if(p) {
		snprintf(ext_out, extout_size, "%s", p);
		return p;
	} else {
		snprintf(ext_out, extout_size, "%s", "");
		return NULL;
	}
}

int replace_ext(char* pathname, const char* new_ext)
{
	char* p;
...
int strip_file(char* pathname, char* path_out, size_t pathout_size)
{
	char* p;

	p = strrchr(pathname, '/');
	if(p) {
		snprintf(path_out, pathout_size, "%.*s", p - pathname, pathname);
		return 0;
	} else {
		snprintf(path_out, pathout_size, "%s", pathname);
		return -1;
	}
}
...
int findcasefile(char* path, char* pattern, char* result, size_t result_size)
{
	DIR* dir;
	...
	dir = opendir(path);
	if(dir == NULL) {
		__android_log_print(ANDROID_LOG_ERROR, "PMDWin", "opendir(%s): %s", path, strerror(errno));
		return -1;
	}

	while((entryp = readdir(dir)) != NULL) {
		if(strcasecmp(entryp->d_name, pattern) == 0) {
			snprintf(result, result_size, "%s/%s", path, entryp->d_name);
			closedir(dir);
			return 0;
		}
	}
...
{
	int		i;
	char	fullpcmfilename[PATH_MAX];

	if(current_dir != NULL) {
		if(findcasefile((char*)current_dir, (char*)filename, fullpcmfilename, sizeof fullpcmfilename) == 0) {
	...
	}while(findcasefile(open_work.pcmdir[i], (char*)filename, fullpcmfilename, sizeof fullpcmfilename) != 0);
...
int PMDWIN::music_load3(uchar *musdata, int size, char *current_dir)
{
	...
	char	pcmfilename[PATH_MAX];
	char	ppsfilename[PATH_MAX];
	char	ppzfilename1[PATH_MAX];
	char	ppzfilename2[PATH_MAX];
	...
	// PPC/P86 読み込み
	_getmemo(pcmfilename, musdata, size, 0);
	resultppc = PMDWIN_OK;

	if(*pcmfilename != '\0') {
		::strip_ext(pcmfilename, ext, sizeof ext);
	...
	// PPS 読み込み
	_getmemo(pcmfilename, musdata, size, -1);
	resultpps = PMDWIN_OK;
	if(*pcmfilename != '\0') {
		// 拡張子を .PPS に変換する
		strcpy(ppsfilename, pcmfilename);
		::replace_ext(pcmfilename, ".pps");
		...
		switch(resultpps) {
			...
			case _ERR_OPEN_PPS_FILE:		break;
			case _ERR_WRONG_PPS_FILE:		return ERR_WRONG_PPS_FILE;
			case _ERR_OUT_OF_MEMORY:		return ERR_OUT_OF_MEMORY;
			default:						return ERR_OTHER;
		}

		if(resultpps == _ERR_OPEN_PPS_FILE) {
	...
	_getmemo(ppzfilename1, musdata, size, -2);
	if(*ppzfilename1 != '\0') {
		::strip_ext(ppzfilename1, ext, sizeof ext);
		if(strcmp(ext, ".PMB") != 0 && strcmp(ext, ".pmb") != 0 && musdata[0] != 0xff) {
...
bool PMDWIN::init(char *path)
{
	...
	if((char *)strrchr(path2, '/') != &path2[strlen(path2)-1]) {
		strcat(path2, "/");
	}
...
//	リズム音の再読み込み
//=============================================================================
bool PMDWIN::loadrhythmsample(char *path)
{
	char	path2[PATH_MAX];
...
//	PCM 検索ディレクトリの設定
//=============================================================================
bool PMDWIN::setpcmdir(char **path)
{
	int		i = 0;
	...
	while(path[i] != NULL && i < MAX_PCMDIR) {
		...
		if((char *)strrchr(open_work.pcmdir[i], '/') !=
							&open_work.pcmdir[i][strlen(open_work.pcmdir[i])-1]) {
			strcat(open_work.pcmdir[i], "/");
		}
...
//	曲の読み込みその１（ファイルから）
//=============================================================================
int PMDWIN::music_load(char *filename)
{
	FILE	*fp;
	int		size, result;
	...
	mstop_f();
	::strip_file(filename, current_dir, sizeof current_dir);
...
//	曲の読み込みその２（メモリから）
//=============================================================================
int PMDWIN::music_load2(uchar *musdata, int size)
{
...
//=============================================================================
char* PMDWIN::_getmemo2(char *dest, uchar *musdata, int size, int al)
{
	char	*buf;
	char	*rslt;
...
//	メモの取得（内部動作、ファイル名から）
//=============================================================================
int PMDWIN::_fgetmemo(char *dest, char *filename, int al)
{
	FILE	*fp;
	uchar	*mmlbuf;
	int		size;
...
//=============================================================================
int PMDWIN::_fgetmemo2(char *dest, char *filename, int al)
{
	char	*buf;
	int		rslt;
...
//=============================================================================
int PMDWIN::_fgetmemo3(char *dest, char *filename, int al)
{
	char	*buf;
	int		rslt;
...
//	メモの取得
//=============================================================================
char* PMDWIN::getmemo(char *dest, uchar *musdata, int size, int al)
{
	char	*buf;
...
//=============================================================================
char* PMDWIN::getmemo2(char *dest, uchar *musdata, int size, int al)
{
	char	*buf;
...
//=============================================================================
char* PMDWIN::getmemo3(char *dest, uchar *musdata, int size, int al)
{
	char	*buf;
...
//	メモの取得（ファイル名から）
//=============================================================================
int	PMDWIN::fgetmemo(char *dest, char *filename, int al)
{
	char	*buf;
	int		rslt;
...
//=============================================================================
int	PMDWIN::fgetmemo2(char *dest, char *filename, int al)
{
	char	*buf;
	int		rslt;
...
//	メモの取得（ファイル名から／半角文字に変換＋ESCシーケンスの除去）
//=============================================================================
int	PMDWIN::fgetmemo3(char *dest, char *filename, int al)
{
	char	*buf;
	int		rslt;
...
//	曲の filename の取得
//=============================================================================
char* PMDWIN::getmusicfilename(char *dest)
{
...
//=============================================================================
char* PMDWIN::getpcmfilename(char *dest)
{
	if(open_work.use_p86) {
...
//	PPC filename の取得
//=============================================================================
char* PMDWIN::getppcfilename(char *dest)
{
	strcpy(dest, open_work.ppcfilename);
	return dest;
}
...
//	PPS filename の取得
//=============================================================================
char* PMDWIN::getppsfilename(char *dest)
{
...
//	P86 filename の取得
//=============================================================================
char* PMDWIN::getp86filename(char *dest)
{
	strcpy(dest, p86drv.p86_file);
	return dest;
}
...
//	PPZ filename の取得
//=============================================================================
char* PMDWIN::getppzfilename(char *dest, int bufnum)
{
...
//	.PPC の読み込み（ファイルから）
//=============================================================================
int PMDWIN::ppc_load(char *filename)
{
	int		resultppc;
...
//	.PPS の読み込み（ファイルから）
//=============================================================================
int PMDWIN::pps_load(char *filename)
{
	int		result;
...
//	.P86 の読み込み（ファイルから）
//=============================================================================
int PMDWIN::p86_load(char *filename)
{
	int		result;
...
//=============================================================================
int PMDWIN::ppz_load(char *filename, int bufnum)
{
	int		result;
...
//	曲の長さの取得(pos : ms)
//=============================================================================
bool PMDWIN::getlength(char *filename, int *length, int *loop)
{
	int		us;
	int		result;
	int		fmwaittemp;
...
//=============================================================================
bool PMDWIN::getlength2(char *filename, int *length, int *loop)
{
	int		us;
	int		result;
	int		fmwaittemp;
...
/*
BOOL DllMain(HINSTANCE hModule,
